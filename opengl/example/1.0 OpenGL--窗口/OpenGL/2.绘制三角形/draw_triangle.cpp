...
//  OpenGL
//
//  Created by xiaerfei on 2022/8/6.
//  Copyright © 2022 Sharon. All rights reserved.
//
/*
 这些API和概念主要是为了解决GPU使用内存和通信效率的问题。
 CPU 和 GPU 之间通信，以及 GPU 从主存里读取数据的效率都是制约系统工作效率的瓶颈，所以使用 glGenBuffer 分配 GPU 里的显存，它不能用指针表示和读取，所以用一个整数 VBO 标识，再用 glBufferData 或 glSubBufferData 把数据从主存传到分配的显存上。
 glBindBuffer 确实是“设置全局变量”，其 GL_ARRAY_BUFFER 表示
 glVertexAttribPointer 所要设置的 buffer，其可以规定读写 buffer 的方式，还可以只使用 buffer 的一部分，这个 buffer 没有直接用 glVertexAttribPointer 的参数指定则是历史遗留问题。
 OpenGL 是 CPU 指挥 GPU 做事，虽然已经通过 VBO 把大块数据送到 GPU 端了，但要绘制很多东西的时候需要反复把各个顶点属性绑定到不同 VBO 上，CPU 和 GPU 间通信也是很耗时的，所以人们设计了 VAO，在 GPU 端保存各顶点属性的绑定状态，要进行相应绘制时只需要绑定一次 VAO，就能自动绑定诸顶点属性，CPU 和 GPU 之间所需要的通信次数大大减少
 */
